...
//Turn around time when the robot hits the horizontal line
        #define TURN_AROUND             500

//Thermistor Temperature Values

//Thermistor HOT Value
    #define HOT                  350

//Thermistor COLD Value
    #define COLD                 250
...
void get_ThermalInput(void); //get the thermistor value
void line_following (void);  //activate the line following portion of code
...
void activate_Arm(void);//activate the arm
void reverse (void);    // drive the robot in reverse
void timing(void);      // Determines how fast the control loop executes
void stop_bot(void);	//stop the robot
void turnAround(void);  //turn the robot 180 degrees
void reverseLittle (void); //reverses the robot for less time
void turnRight(void); //turns the robot 90 degrees right
void turnLeft(void); // turns the robot 90 degrees left
void cold_Pale(void); // do this if pale is cold
void warm_Pale(void); //do this if pale is room temperature
void hot_Pale(void); //do this is pale is hot
...
unsigned int thermValue; // RA1/AN1: Thermistor value
...
int main(void)
{
        ...
        LATBbits.LATB13=0;
            ...
            line_following();
            //stop the robot when it hits the horizontal line
            stop_bot();
            
            reverseLittle(); //reverse for 250 ms so the pale is lifted
                            //correctly
            //lift the pale
            activate_Arm();
            //get the thermal value before lifting for a more accurate value
            get_ThermalInput();

            //if the pale is cold
            if ((thermValue < COLD) && (thermValue >= 0))
            {
                cold_Pale();
            }

            else if ((thermValue > COLD) && (thermValue < HOT))
            {
                warm_Pale();
            }

            else if ((thermValue > HOT) && (thermValue < 2000))
            {
                hot_Pale();
            }
    //ignore the first black line and stop at the second line
    LATBbits.LATB14=1;
    LATBbits.LATB15=0;
    LATBbits.LATB12=1;
    LATBbits.LATB13=0;
...
void initialize(void)
{
  ...
  // initialize pin for thermistor

  ANSAbits.ANSA1 = 1;    //Make RA1/AN1 input Analog
...
/*******************************************************************************
* Function:        void get_ThermalInput(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 * Side Effects:    None
 *
 * Overview:        Obtains input information either on-chip
 *                  (from internal registers, etc...) or off-chip
 *                  (pin voltage levels). Uses this information to use the
 *                  thermistor input
 *
 * Note:            None
 ******************************************************************************/

void get_ThermalInput(void)
{
  //Sample/save thermistor connected to RA1/AN1
  delayMs (2500);
  AD1CHS = 0x0001;           // Connect RA1/AN1 as CH0 input
  AD1CON1bits.SAMP = 1;      // Sample thermistor value
  delayMs(50);              // After 50mS start conversion
  AD1CON1bits.SAMP= 0 ;      // Convert thermistor value
  while (!AD1CON1bits.DONE); // conversion done? (takes 12*Tad)
  thermValue = ADC1BUF0;     //yes, then save ADC value
}
...
/*******************************************************************************
* Function:        void reverseLittle(void)
*
* PreCondition:    None
*
* Input:           None
 ...
 * Side Effects:    None
 *
 * Overview:        This function stops the robot's movement
 *
 * Note:            None
 ******************************************************************************/
void stop_bot(void)
...
/*******************************************************************************
 * Function:        void line_following(void)
 *
 * PreCondition:    Sensors not on black line
 *
 * Input:           None
 *
 * Output:          None
 ...
 * Side Effects:    None
 *
 * Overview:        This function activates the line following program
 *
 * Note:            None
 ******************************************************************************/
...
* Side Effects:    None
*
* Overview:        This function turns the robot 180 degrees
*
* Note:            None
*******************************************************************************/
...
* Side Effects:    None
*
* Overview:        This function turns the robot right 90 degrees
*
* Note:            None
*******************************************************************************/
void turnRight(void)
...
* Side Effects:    None
*
* Overview:        This function turns the robot left 90 degrees
*
* Note:            None
*******************************************************************************/
    void turnLeft(void)
...
* Side Effects:    None
*
* Overview:        This function tells the robot what to do if the pale is hot
*
* Note:            None
*******************************************************************************/
...
void hot_Pale(void)
{
        ...
        //then turn left
        turnLeft();
        get_Inputs(); //inputs to activate line-follow

        line_following();
        //stop the robot and drop the pale
        //then reverse and turn around
        stop_bot();
        activate_Arm();
        reverse();
        turnAround();
        //keep line following until the middle line
        line_following();
        //drive forward to ignore line
        LATBbits.LATB14=1;
        LATBbits.LATB15=0;
        LATBbits.LATB12=1;
        LATBbits.LATB13=0;
        delayMs (250);
        //turn right and get inputs
        turnRight();
        get_Inputs();
        //keep line following till the eng
        line_following();
}
/*******************************************************************************
* Function:        void warm_Pale(void)
*
* PreCondition:    None
*
* Input:           None
...
* Side Effects:    None
*
* Overview:        This function tells the robot what to do if the pale is warm
*
* Note:            None
*******************************************************************************/
...
void warm_Pale (void)
{
        //drive forward to ignore middle line
        LATBbits.LATB14=1;
        LATBbits.LATB15=0;
        LATBbits.LATB12=1;
        LATBbits.LATB13=0;
        delayMs (1000);
        //get inputs to activate line-following
        get_Inputs();

        line_following();

        //stop the robot, drop the pale
        //then reverse and turn around
        stop_bot();
        activate_Arm();
        reverse();
        turnAround();
        //keep line following till it hits the
        //black line
        line_following();
        ...
        delayMs (500);
        get_Inputs();
        //line follow till the end
        line_following();
}
/*******************************************************************************
* Function:        void cold_Pale(void)
*
* PreCondition:    None
*
* Input:           None
...
* Side Effects:    None
*
* Overview:        This function tells the robot what to do if the pale is cold
*
* Note:            None
*******************************************************************************/
...
void cold_Pale (void)
{
        ...
        LATBbits.LATB13=0;
        delayMs (500);
        //turn right and get inputs to activate line
        //following
        turnRight();
        get_Inputs();
        //line follow till the end
        line_following();
            //stop and drop the pale and turn around
            stop_bot();
            activate_Arm();
            reverse();
            turnAround();
            //line follow till the middle line
            line_following();
        ...
        LATBbits.LATB13=0;
        delayMs (250);
        //turn left and line follow till the end
        turnLeft();
        get_Inputs();
